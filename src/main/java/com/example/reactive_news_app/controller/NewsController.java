...
public class NewsController {
    ...
    public Flux<NewsArticle> getSlowNews() {
        ...
        return newsService.getAllNews()
                .delayElements(Duration.ofSeconds(1)) // 각 요소마다 1초 지연
    ...
    public Flux<NewsArticle> getNewsStreamWithBackpressure() {
        log.info("GET /api/news/stream-backpressure - 백프레셔 처리된 뉴스 스트림");

        return Flux.interval(Duration.ofMillis(100)) // 빠른 생성 (100ms마다)
                .map(this::generateTestArticle)
                .onBackpressureBuffer(10) // 최대 10개 버퍼링
